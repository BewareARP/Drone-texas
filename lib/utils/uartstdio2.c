...
// disable echo by calling UARTEchoSet2(false).
//
//*****************************************************************************
...
#define TX_BUFFER_EMPTY2         (IsBufferEmpty(&g_ui32UARTTxReadIndex,   \
                                               &g_ui32UARTTxWriteIndex))
#define TX_BUFFER_FULL2          (IsBufferFull(&g_ui32UARTTxReadIndex,  \
                                              &g_ui32UARTTxWriteIndex, \
                                              UART_TX_BUFFER_SIZE2))
...
//! functions: UARTprintf2() or UARTgets2().  This function assumes that the
...
//! in buffered mode, a call to UARTPeek2() may be made to determine whether
...
//! received from the UART but not yet read using UARTgets2().
//!
//! \return None.
//
//*****************************************************************************
...
//! UARTprintf2() that is waiting to be transmitted.  On return, the transmit
        ...
        while(MAP_UARTCharsAvail(g_ui32Base))
        {
            ...
            if(!g_bDisableEcho)
            {
                ...
                if((cChar == '\r') || (cChar == '\n') || (cChar == 0x1b))
                {
                    ...
                    // UARTgets2() where the line ends.  We also send an
