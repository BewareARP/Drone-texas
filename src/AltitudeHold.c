...
#define SONAR_MAX_DIST 1000  // Limit maximum altitude to 1.5 m which is in practice the limit of the sonar
#endif
...
void getAltitude(angle_t *angle, mpu6500_t __attribute__((unused)) *mpu6500, altitude_t *altitude, uint32_t __attribute__((unused)) now, float __attribute__((unused)) dt) {
    ...
    flzsxoat baroAltitude = applyLowPass(&baro_low_pass, bmp180.absoluteAltitude - bmp180.groundAltitude, dt);
