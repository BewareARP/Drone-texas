...
//
//*****************************************************************************
static unsigned char g_pcUARTTxBuffer[UART_TX_BUFFER_SIZE1];
...
//
//*****************************************************************************
static unsigned char g_pcUARTRxBuffer[UART_RX_BUFFER_SIZE1];
...
//
//*****************************************************************************
#define TX_BUFFER_USED1          (GetBufferCount(&g_ui32UARTTxReadIndex,  \
                                                &g_ui32UARTTxWriteIndex, \
                                                UART_TX_BUFFER_SIZE1))
#define TX_BUFFER_FREE1          (UART_TX_BUFFER_SIZE1 - TX_BUFFER_USED1)
#define TX_BUFFER_EMPTY1         (IsBufferEmpty(&g_ui32UARTTxReadIndex,   \
                                               &g_ui32UARTTxWriteIndex))
...
//
//*****************************************************************************
#define RX_BUFFER_USED1          (GetBufferCount(&g_ui32UARTRxReadIndex,  \
                                                &g_ui32UARTRxWriteIndex, \
                                                UART_RX_BUFFER_SIZE1))
#define RX_BUFFER_FREE1          (UART_RX_BUFFER_SIZE - RX_BUFFER_USED1)
#define RX_BUFFER_EMPTY1         (IsBufferEmpty(&g_ui32UARTRxReadIndex,   \
                                               &g_ui32UARTRxWriteIndex))
#define RX_BUFFER_FULL1          (IsBufferFull(&g_ui32UARTRxReadIndex,  \
                                              &g_ui32UARTRxWriteIndex, \
                                              UART_RX_BUFFER_SIZE1))
...
UARTPrimeTransmit(uint32_t ui32Base)
{
    ...
    if(!TX_BUFFER_EMPTY1)
    {
        ...
        while(MAP_UARTSpaceAvail(ui32Base) && !TX_BUFFER_EMPTY1)
        {
            ...
            ADVANCE_TX_BUFFER_INDEX1(g_ui32UARTTxReadIndex);
        }
    ...
    for(uIdx = 0; uIdx < ui32Len; uIdx++)
    {
        ...
        if(pcBuf[uIdx] == '\n')
        {
            if(!TX_BUFFER_FULL)
            {
                ...
                ADVANCE_TX_BUFFER_INDEX1(g_ui32UARTTxWriteIndex);
            }
            else
            {
                //
...
UARTgets1(char *pcBuf, uint32_t ui32Len)
{
    ...
    while(1)
    {
        ...
        if(!RX_BUFFER_EMPTY1)
        {
            ...
            ADVANCE_RX_BUFFER_INDEX1(g_ui32UARTRxReadIndex);
    ...
    if(ui32Ints & (UART_INT_RX | UART_INT_RT))
    {
        ...
        while(MAP_UARTCharsAvail(g_ui32Base))
        {
            ...
            if(!g_bDisableEcho)
            {
                ...
                if(cChar == '\b')
                {
                    ...
                    if(!RX_BUFFER_EMPTY1)
                    {
                        ...
                        {
                            g_ui32UARTRxWriteIndex = UART_RX_BUFFER_SIZE1 - 1;
                        }
                        else
                        {
                            g_ui32UARTRxWriteIndex--;
            ...
            if(!RX_BUFFER_FULL1)
            {
                ...
                ADVANCE_RX_BUFFER_INDEX1(g_ui32UARTRxWriteIndex);
